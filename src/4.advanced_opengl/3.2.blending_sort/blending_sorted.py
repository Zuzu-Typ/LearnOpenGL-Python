...
def main() -> int:
    ...
    transparentVertices = glm.array(glm.float32,
    ...
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * glm.sizeof(glm.float32), ctypes.c_void_p(3 * glm.sizeof(glm.float32)))
    # transparent VAO
    transparentVAO = glGenVertexArrays(1)
    transparentVBO = glGenBuffers(1)
    glBindVertexArray(transparentVAO)
    glBindBuffer(GL_ARRAY_BUFFER, transparentVBO)
    glBufferData(GL_ARRAY_BUFFER, transparentVertices.nbytes, transparentVertices.ptr, GL_STATIC_DRAW)
    glEnableVertexAttribArray(0)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * glm.sizeof(glm.float32), None)
    glEnableVertexAttribArray(1)
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * glm.sizeof(glm.float32), ctypes.c_void_p(3 * glm.sizeof(glm.float32)))
    ...
    transparentTexture = loadTexture("window.png")
    ...
    while (not glfwWindowShouldClose(window)):
        ...
        # ---------------------------------------------
        windows_by_distance = {}
        for windowCoord in windows:
            distance = glm.length(camera.Position - windowCoord)
            windows_by_distance[distance] = windowCoord
        ...
        for distance in sorted(windows_by_distance):
            model = glm.mat4(1.0)
            model = glm.translate(model, windows_by_distance[distance])
            shader.setMat4("model", model)
            glDrawArrays(GL_TRIANGLES, 0, 6)
