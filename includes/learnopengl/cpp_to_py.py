...
GL_GEN_PATTERN = "(glGen(?:VertexArrays|Buffers))\\(([0-9]+),\\s*&?(\\w+)\\)"
...
for file_path in sys.argv[1:]:
    ...
    file_content = replace_with_func(file_content, IF_ELSE_WHILE_PATTERN, lambda m: f"{m.group()}:")
    ...
    file_ = open(file_path.replace(".h", ".py").replace(".cpp", ".py"), "w")
